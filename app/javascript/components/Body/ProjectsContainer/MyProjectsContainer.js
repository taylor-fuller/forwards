...
const MyProjectsContainer = (props) => {
    ...
    function renderProjectsLed() {
        ...
        if (props.projects_led && props.projects_led.length >= 1) {
            projectsLed = props.projects_led.map(project => <div key={project.id} id={project.id} className='project-item' onClick={() => handleProjectSelect(project.id, project.name, project.team_id, returnTeamName(project.team_id))}><h3 title={project.name}>{project.name}</h3> <h3 className={project.tasks.overdue.length === 0 ? "grey" : 'red'}>{project.tasks.overdue.length}</h3> <h3 className={project.tasks.due_today.length === 0 ? "grey" : 'red'}>{project.tasks.due_today.length}</h3> <h3 className={project.tasks.due_soon.length === 0 ? "grey" : 'orange'}>{project.tasks.due_soon.length}</h3> <h3 className={project.tasks.all_tasks.length-project.tasks.completed.length === 0 ? "grey" : null}>{project.tasks.all_tasks.length-project.tasks.completed.length}</h3> <h3>{isNaN(Math.round((project.tasks.completed.length/project.tasks.all_tasks.length)*100)) ? '0' : (Math.round((project.tasks.completed.length/project.tasks.all_tasks.length)*100))}%</h3> <h3 title={returnTeamName(project.team_id)}>{returnTeamName(project.team_id)}</h3></div>)
            return(
                <Fragment>
                    <h2>Projects Led</h2>
